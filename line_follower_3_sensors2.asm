; codigo que detecta linha quando nivel logico é 0
...
setup:
    ...
    ;configura pwm
    BANKSEL PR2
    MOVLW D'255'      ; PR2 = 255 (período máximo)
    MOVWF PR2         ; Define o período do PWM
    BANKSEL CCPR1L
    MOVLW D'204'      ; CCPR1L = 204 (parte alta do duty cycle)
    MOVWF CCPR1L      ; Define o valor do duty cycle
    MOVLW B'00111100' ; DC1B<1:0> = 11 (parte baixa do duty cycle)
    MOVWF CCP1CON     ; Configura o modo PWM
    MOVLW B'00000110' ; Timer2 ligado, prescaler 1:16
    MOVWF T2CON       ; Configura o Timer2
...
loop: 
    ...
    MOVLW   0x02	;para direita quando leitura=2
    XORWF   leitura,W
    BTFSC   STATUS,Z
    CALL    para_direita
    
    MOVLW   0x01	;para esquerda quando leitura=1
    XORWF   leitura,W
    BTFSC   STATUS,Z
    CALL    para_esquerda
